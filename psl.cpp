...
int main() {
  ...
  string name[6] = {"Karachi Kings",     "Lahore Qalandars",
      ...
      hcity[6] = {"Karachi",   "Lahore", "Peshawar",
                  "Islamabad", "Quetta", "Multan Sultans"},  // array of cities
  ...
  ,
      ...
      teamA[6] = {"Karachi Kings",    "Lahore Qalandars",  "Peshawar Zalmi",
         ...
         teamB[6] = {"Karachi Kings",    "Lahore Qalandars",  "Peshawar Zalmi",
                     "Islamabad United", "Quetta Gladiators", "Multan Sultans"},
         man_of_match[15] = {" "};

  int price[6] = {-1}, score1[17] = {-1}, score2[17] = {-1},
      man_of_match_score[15] = {0}, points[6] = {0}, mplayed[6] = {0},
      won[6] = {0}, loss[6] = {0}, draw[6] = {0}, count = 0, scount = 0, ii = 0,
      jj = 0;
  ...
  while (flag) {
    ...
    } else if (choice == 2)  // bidding
    {
      ...
      {
        int price[6] = {-1};  // array of price for a player from each team
        cout << "Player:- ";
        cout << pname[i];
        cout << endl;
        for (int j = 0; j < 6; j++)  // loop for teams
        {
          ...
          {
            for (int l = 0; l < 6; l++) {
              if ((price[j] == price[l] && l != j) || price[j] < 0) {
                cout << "Team enter yout current Bid(Bid amount "
          ...
          max = -1;
          mindex = -1;
          for (int j = 0; j < 6; j++)  // finding which team bid the most
          {
            if (price[j] > max) {
              max = price[j];
              mindex = j;
      ...
      if (!draft) {
        ...
        for (int i = ii; i < 6; i++) {
          ...
          for (int j = i + 1; j < 6; j++) {
            ...
            if (i != j && (score1[scount] == -1 && score2[scount] == -1)) {
              cout << "Match " << scount << ":- \n";
              cout << teamA[i] << " vs " << teamB[j] << endl;
              cout << "Enter the score of " << teamA[i] << ":- \n";
              cin >> score1[scount];  // score of team 1
              while (inflag) {
                if (score1[scount] < 0) {
              ...
              cin >> score2[scount];  // score of team 2
              while (inflag) {
                if (score2[scount] < 0) {
                  cout << "The score cannot be negative, "
                          "Again "
              ...
              inflag = true;
              if (score1[scount] > score2[scount])
              ...
              // ka izafa ho ga)
              {
                mplayed[i] += 1;  // counter for match played by team 1
                won[i] += 1;
                loss[j] += 1;
                mplayed[j] += 1;  // counter for match played by team 2
                points[i] += 2;
                cout << "Team won:- " << teamA[i] << endl
                     << " Points:- " << points[i] << endl;
              } else if (score1[scount] <
                         score2[scount])  // to determine the
                             ...
                             // hai kai kis team kai pas kon sa player hai
    {
      ...
      for (int i = 0; i < 6; i++) {
        ...
        cout << name[i];
        int spaces =
            20 - name[i].length();  // Ensure consistent alignment for names
        while (spaces--) cout << " ";
